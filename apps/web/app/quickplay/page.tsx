...
export default function QuickPlayPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await quickPlay({ displayName, playerCount, targetScore });
      saveSession({
        roomId: res.roomId,
    ...
    <main className="min-h-screen bg-green-900 text-white p-8 flex items-center justify-center">
      ...
      >
        <div className="text-center mb-2">
          ...
          <h1 className="text-2xl font-bold">Brza igra</h1>
          <p className="text-sm text-zinc-400 mt-1">
            Ulazi za sekunde — protivnici čekaju.
          </p>
        </div>
        ...
        >
          {loading ? "Tražim sto..." : "Igraj odmah →"}
        </button>
      </form>
    </main>
  );
